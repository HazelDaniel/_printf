...
char **get_flags(char *format, int *ind_ptr)
{
	...
	if (result == NULL)
		return (result);
	result[0] = malloc(2 * sizeof(char)), result[1] = malloc(6 * sizeof(char));
	...
	for (j = 0; j < 6; j++, i++)
	{
		while ((flags_h_ret = is_flag(format[i], flags_hash),
			flags_h_ret[1] != -1) && j < 6)
		{
			if (flags_h_ret == NULL)
			{
				*status = '0', free(flags_h_ret);
				return (NULL);
			}
			flag_bool = flags_h_ret[0], flag_index = flags_h_ret[1];
			if ((!flags_hash[flag_index].found))
			{
				flags_hash[flag_index].found = 1, result[1][k] = format[i], k++;
			}
			i++, j++, free(flags_h_ret);
		}
		break;
	}
	for (; k < 6; k++)
		result[1][k] = '\0';
	*ind_ptr = --i;/* there may be a bug with index here or the next.*/
	--j, result[0][0] = *status, result[0][1] = '\0';
	return (result);
}
